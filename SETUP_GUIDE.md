...
- `init.sql` : File kịch bản khởi tạo bảng và seed dữ liệu mẫu cho MySQL.
...
- **Docker Desktop:** Dùng để chạy cơ sở dữ liệu MySQL mà không cần cài trực tiếp lên máy.
- **Git:** Dùng để tải mã nguồn.
- **DBeaver** (hoặc MySQL Workbench): Để quản lý Database.

## ⚙️ Bước 2: Tải mã nguồn và Chạy Backend (Bao gồm Database)
---

Dự án này sử dụng **Docker Compose** để đóng gói và tự động chạy cả Database (MySQL) và Backend API cùng lúc. Mở Terminal và làm theo các bước:
...
Mặc dù Database đã được tạo thông qua Docker Compose, bạn vẫn cần nạp cấu trúc bảng và dữ liệu ban đầu:
- Mở **DBeaver** (hoặc MySQL Workbench), tạo kết nối mới (MySQL).
- Server Host: `localhost` | Port: `3307` *(Đã cấu hình đổi port để tránh xung đột)*
- Database: `DoAnNhom_Db`
- Username: `root` | Password: `Aa123456`
- Mở file `init.sql` có trong thư mục gốc của dự án, bôi đen toàn bộ và nhấn **Alt + X** (hoặc nút Run Script) để tạo các bảng và dữ liệu mẫu.
...
**2. Lỗi mất kết nối Database (Connection Refused) hoặc trùng Port**
- **Vấn đề:** Backend báo lỗi không thể kết nối tới DB, hoặc lệnh Docker Compose bị lỗi `Bind for 0.0.0.0:3306 failed: port is already allocated`.
- **Giải pháp:** 
  - Đảm bảo Docker Desktop đang bật và hoạt động.
  - **Lưu ý quan trọng (Cái bạn hay quên):** Bạn phải **tắt phần mềm XAMPP, WAMP hoặc dịch vụ MySQL đang chạy ẩn dưới máy** (vào `services.msc` tìm `MySQL` và nhấn Stop). Nếu có phần mềm khác đang chiếm cổng, Docker sẽ không thể khởi tạo thành công kết nối.
  - Nhớ rằng tài khoản DB của đồ án là `root` và mật khẩu là `Aa123456` (truy cập qua port `3307`).
